...
import com.isti.traceview.gui.GraphPanel.CURSOR_TYPE;
...
public class ViewPSD extends JDialog implements PropertyChangeListener, ChartProgressListener {
	...
	public ViewPSD(Frame owner, List<Spectra> data, TimeInterval ti, Configuration configuration, List<IChannel> input) {
		...
		if (isVisible() && (e.getSource() == optionPane) && (prop.equals(JOptionPane.VALUE_PROPERTY))) {
			...
			if (value.equals("Close")) {
				...
				try {
					...
					JOptionPane.showMessageDialog(XMAXframe.getInstance(), "Can't find Dump frequencies file", "Error", JOptionPane.ERROR_MESSAGE);
	...
	private XYSeriesCollection createDataset(List<Spectra> ds) {
		...
		XYSeries lowNoiseModelSeries = new XYSeries("MODEL_LOW");
		XYSeries highNoiseModelSeries = new XYSeries("MODEL_HIGH");
	...
	private MyOptionPane createChartPanel(XYDataset dataset, List<IChannel> input) {
		chart = ChartFactory.createXYLineChart(null, // title
				"Period, s", // x-axis unitL
				"Power spectra density", // y-axis unitL
				dataset, // data
	...
	public class PSDSlotFactory implements ISlotFactory {
		public int getInfoAreaWidth(){
			return 0;
		}
		public Slot getSlot(List<? extends IChannel> channels) {
			return new Slot(channels, getInfoAreaWidth(), false);
		}

		public Slot getSlot(IChannel channel) {
			return new Slot(channel, getInfoAreaWidth(), false);
		}
	}
	...
	class MyOptionPane extends JPanel implements Printable {
		...
		private List<IChannel> input;

		public MyOptionPane(TraceViewChartPanel cp, List<IChannel> input) {
			this.cp = cp;
			this.input = input;
