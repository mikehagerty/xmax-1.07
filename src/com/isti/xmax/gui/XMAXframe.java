...
public class XMAXframe extends JFrame implements MouseInputListener, ActionListener, ItemListener, Observer {
	...
	private void initialize() {
		...
		graphPanel.setCursorType(XMAX.getConfiguration().getShowBigCursor()?CURSOR_TYPE.CROSS:CURSOR_TYPE.STANDARD);
		showBigCursorMenuCheckBox.setState(XMAX.getConfiguration().getShowBigCursor());
		...
		try {
			graphPanel.setChannels(dm.getNextCnannelSet());
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			List<Slot> selectedViews = graphPanel.getSelectedSlots();
			for (Slot channelView: selectedViews) {
	...
	class NextAction extends AbstractAction implements Action {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				XMAXDataModule dm = XMAX.getDataModule();
				graphPanel.setChannels(dm.getNextCnannelSet()); // and also switch off OVR,
																		// SEL, ROT
	...
	class PreviousAction extends AbstractAction implements Action {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				XMAXDataModule dm = XMAX.getDataModule();
				graphPanel.setChannels(dm.getPreviousCnannelSet());
				graphPanel.setScaleMode(new ScaleModeAuto());
				graphPanel.setMeanState(new MeanModeDisabled());
				graphPanel.setOffsetState(new OffsetModeDisabled());
				graphPanel.setPickState(false);
	...
	class ShowBigCursorAction extends AbstractAction implements Action {
		...
		public void actionPerformed(ActionEvent e) {
			graphPanel.setCursorType(showBigCursorMenuCheckBox.getState()?CURSOR_TYPE.CROSS:CURSOR_TYPE.STANDARD);
			statusBar.setMessage("");
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (selectedChannels.size() == 0) {
					...
					List<IChannel> channelSet = graphPanel.getChannels();
					Station station = null;
					IChannel Nchannel = null;
					IChannel Echannel = null;
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (selectedViews.size() == 0) {
					selectedViews = graphPanel.getSlots();
				}
				for (Slot channelView: selectedViews) {
					selectedChannels.addAll(channelView.getPlotDataProviders());
				}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (fc.showSaveDialog(XMAXframe.getInstance()) == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					if (selectedChannels.size() > 0) {
						Iterator<IChannel> it = selectedChannels.iterator();
						while (it.hasNext()) {
