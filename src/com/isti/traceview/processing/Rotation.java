...
public class Rotation {
	 ...
	 */
	public PlotData rotate(IChannel channel, TimeInterval ti, int pointCount, IFilter filter, IColorModeState colorMode) throws TraceViewException {
		...
		PlotData ret = new PlotData(channel.getName(), channel.getUnitName(), channel.getColor(), channel.getScale());
	 ...
	 * @throws TraceViewException
	 */
	public List<Segment> rotate(IChannel channel, TimeInterval ti) throws TraceViewException {
		IChannel[] triplet = getChannelsTriplet(channel, ti);
		char channelType = channel.getType();
	 ...
	 * @return flag
	 */
	public static boolean isComplementaryChannelExist(IChannel channel, TimeInterval ti) {
		try {
			IChannel[] triplet = getChannelsTriplet(channel, ti);
	 ...
	 * @throws TraceViewException
	 */
	public static IChannel[] getChannelsTriplet(IChannel channel, TimeInterval ti) throws TraceViewException {
		IChannel[] chs = new PlotDataProvider[3];
		char channelType = channel.getType();
		if (channelType == 'E' || channelType == '2') {
			chs[0] = channel;
			try{
	...
	private static IChannel getComplementaryChannel(IChannel channel, char channelType) throws TraceViewException {
		String channelName = channel.getChannelName().substring(0, channel.getChannelName().length() - 1) + channelType;
		IChannel channelComplementary = TraceView.getDataModule().getChannel(channelName, channel.getStation(), channel.getNetworkName(),
	...
	private static PlotData getComplementaryPlotData(IChannel channel, char channelType, TimeInterval ti, int pointCount, IFilter filter, IColorModeState colorMode)
			throws TraceViewException {
